...
function renderHandouts(){
  ...
  const filter=panel.dataset.handoutFilter||'all',all=handoutList(),arr=all.filter(h=>{if(filter==='all')return true;if(filter==='new')return !handoutAcks[h.id]?.viewedAt;if(filter==='confirmed')return !!handoutAcks[h.id]?.confirmedAt;return true}),newCount=all.filter(h=>!handoutAcks[h.id]?.viewedAt).length,state=$('dcxOpenHandouts')?.querySelector('.dcxModuleState');if(state)state.textContent=newCount?`${newCount} NOVO${newCount===1?'':'S'}`:'ABRIR';if(!arr.length){box.innerHTML=`<div class="localEmptyState"><b>${filter==='new'?'NENHUM HANDOUT NOVO':filter==='confirmed'?'NENHUM HANDOUT CONFIRMADO':'NENHUM HANDOUT RECEBIDO'}</b><small>Os documentos publicados pelo Keymaster aparecerão aqui.</small></div>`;return}
