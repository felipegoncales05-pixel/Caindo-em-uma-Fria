...
let ctx=null,publicCreds={},npcCreds={},avatarUrls={},handouts={},handoutAcks={},handoutRefs=[],ackRefs=new Map(),teamId='',initTimer=null,credRefs=[];
function identity(){
...
async function sendHandoutFromForm(){
  ...
  const title=String($('kmHandoutTitle')?.value||'').trim(),description=String($('kmHandoutDescription')?.value||'').trim(),type=$('kmHandoutTargetType')?.value||'all',raw=String($('kmHandoutTarget')?.value||'').trim(),file=$('kmHandoutFile')?.files?.[0],externalUrl=validateExternalUrl($('kmHandoutExternalUrl')?.value||'');
  if(!title)throw new Error('Informe um título.');if(!file&&!externalUrl)throw new Error('Selecione uma imagem/texto pequeno OU informe um LINK EXTERNO.');if(file&&externalUrl)throw new Error('Use arquivo OU link externo, não os dois ao mesmo tempo.');
  ...
  const id=`H-${Date.now()}-${Math.random().toString(36).slice(2,7).toUpperCase()}`,inline=file?await prepareInlineFile(file):null;
  const meta={id,title,description,targetType:type,targetId,fileName:inline?.fileName||(externalUrl?'LINK EXTERNO':'HANDOUT'),fileType:inline?.fileType||(externalUrl?'text/uri-list':''),fileSize:inline?.fileSize||0,dataUrl:inline?.dataUrl||null,externalUrl:externalUrl||null,storedMode:inline?.storedMode||(externalUrl?'external-link':'metadata'),createdAt:firebase.database.ServerValue.TIMESTAMP,createdBy:'KEYMASTER',archived:false};
...
async function openHandout(id){const h=handouts[id];if(!h)return;try{await ack(id,{viewedAt:firebase.database.ServerValue.TIMESTAMP});if(h.externalUrl){window.open(h.externalUrl,'_blank','noopener,noreferrer');return}if(h.dataUrl){const u=dataUrlToObjectUrl(h.dataUrl),w=window.open(u,'_blank');setTimeout(()=>URL.revokeObjectURL(u),60000);if(!w)location.href=u;return}throw new Error('Este Handout não possui conteúdo disponível.')}catch(e){alert('Falha ao abrir Handout: '+(e?.message||e))}}
...
function bindKeymasterHandout(){
  if(role()!=='host')return;const btn=$('kmHandoutSend');if(!btn||btn.dataset.noStorageBound==='1')return;btn.dataset.noStorageBound='1';btn.disabled=false;btn.classList.add('kmHandoutSendLive');btn.innerHTML='<b>ENVIAR AOS JOGADORES</b><small>RTDB // SEM STORAGE</small>';btn.title='Enviar imagem/texto pequeno ou link externo pelo RTDB';btn.addEventListener('click',async()=>{if(btn.disabled)return;btn.disabled=true;const old=btn.innerHTML;btn.innerHTML='<b>ENVIANDO...</b><small>RTDB</small>';try{const h=await sendHandoutFromForm();alert(`Handout enviado: ${h.title}`);$('kmHandoutClear')?.click();if($('kmHandoutExternalUrl'))$('kmHandoutExternalUrl').value=''}catch(e){console.error(e);alert('Falha ao enviar Handout: '+(e?.message||e))}finally{btn.disabled=false;btn.innerHTML=old}})
}
async function boot(){ctx=context();if(!ctx)return false;await startCredentialListeners();if(role()==='player')attachPlayerHandouts();else bindKeymasterHandout();return true}
function tick(){const c=context();if(!c)return;if(!ctx||ctx.uid!==c.uid||ctx.room!==c.room){ctx=c;startCredentialListeners();if(role()==='player')attachPlayerHandouts();else bindKeymasterHandout()}else if(role()==='player'){const t=currentTeam();if(t!==teamId)attachPlayerHandouts()}}
window.DCX=window.DCX||{};window.DCX.Media={getAvatarUrl,credentialMeta,uploadOwnAvatar,removeOwnAvatar,uploadAdminAvatar,removeAdminAvatar,sendHandoutFromForm,refreshHandouts:attachPlayerHandouts};
