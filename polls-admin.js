...
(() => {
  ...
  let db=null, auth=null, room="FRIA-01", base="", polls={}, pollVotes={}, pollRef=null, voteRef=null, publicPollRef=null, publicPollSignature="", retryTimer=null, tickTimer=null, editId="", initialized=false;
  ...
  function voteCounts(p){const out={};optionEntries(p).forEach(o=>out[o.id]=0);const pid=p?.id||"";const hasLive=pid&&Object.prototype.hasOwnProperty.call(pollVotes,pid);const src=hasLive?(pollVotes?.[pid]||{}):(p?.votes||{});Object.values(src||{}).forEach(v=>{const id=typeof v==="string"?v:(v?.choiceId||v?.optionId);if(id&&Object.prototype.hasOwnProperty.call(out,id))out[id]++});return out}
  ...
  async function deletePoll(id){const p=getPoll(id);if(!p||!confirm(`Excluir definitivamente a votação “${p.title||id}”?\n\nEssa ação remove também opções, resultado e votos armazenados.`))return;await Promise.all([ref(id).remove(),db.ref(`rooms/${room}/dcx/pollVotes/${id}`).remove()]);if(editId===id)resetForm();toast("VOTAÇÃO EXCLUÍDA")}
