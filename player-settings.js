(()=>{
  ...
  function apply(updateUI=true){
    document.documentElement.dataset.playerTheme=state.theme;
    document.documentElement.dataset.playerDensity=state.density;
    if(updateUI)syncUI();
  }
  ...
  function syncUI(){
    document.querySelectorAll('[data-theme-choice]').forEach(btn=>{
      ...
      const badge=btn.querySelector('.settingsSelected');
      if(badge)badge.textContent=active?'ATIVO':'USAR';
    });
    document.querySelectorAll('[data-density-choice]').forEach(btn=>{
      const active=btn.dataset.densityChoice===state.density;
      btn.classList.toggle('active',active);
      btn.setAttribute('aria-pressed',String(active));
    });
    ...
    if(label){
      const names={
        'arc-cyan':'ARC // CIANO',
        'dcx-green':'DCX // VERDE',
        'ember-amber':'EMBER // ÂMBAR',
        'void-violet':'VOID // VIOLETA'
      };
      label.textContent=`${names[state.theme]} // ${state.density==='compact'?'COMPACTO':'CONFORTÁVEL'}`;
    }
  }

  function setTheme(theme){
    if(!THEMES.includes(theme))return;
    state.theme=theme;persist();apply();
  }

  function setDensity(density){
    if(!DENSITIES.includes(density))return;
    state.density=density;persist();apply();
  }

  function reset(){
    state={...DEFAULTS};persist();apply();
  }

  function toggle(force){
    const panel=$('dcxSettingsPanel');if(!panel)return;
    const open=typeof force==='boolean'?force:panel.classList.contains('hidden');
    panel.classList.toggle('hidden',!open);
    panel.setAttribute('aria-hidden',String(!open));
    if(open){syncUI();setTimeout(()=>$('settingsCloseBtn')?.focus(),0)}
  }

  function bind(){
    $('dcxOpenSettings')?.addEventListener('click',e=>{e.preventDefault();toggle(true)});
  ...
  function init(){
    apply(false);bind();syncUI();
    console.info('[DCX OS] PLAYER SETTINGS V1 // 4 THEMES + DENSITY // LOCAL ONLY');
  }

  window.DCX=window.DCX||{};
  window.DCX.PlayerSettings={toggle,setTheme,setDensity,reset,get state(){return {...state}}};
